...
class Seguimiento extends Controllers{
    ...
    public function getProspectos(string $nomConexion)
    {
        $arrRes = [];
        if($nomConexion == 'all')
        {
            foreach (conexiones as $key => $conexion) {
                if($key != 'bd_user')
                {
                    $arrData = $this->model->selectProspectos($key);
                    if(count($arryData) > 0)
                    {
            ...
            $newArray = $this->model->selectProspectos($nomConexion);
        ...
        for ($i=0; $i < count($newArray) ; $i++) { 
            ...
            <div class="btn-group">
                ...
                <i class="fas fa-aley-group"></i> &nbps; Acciones
                </button>
                <div class="dropdown-menu">
                    <button class="dropdown-item btn btn-outline-secondary btn-sm btn-flat icono-color-principal btnVerProspectos" con="'.$newArray[$i]['nom_conexion'].'" data-toggle="modal" data-target="#ModalAgendarProspectoSeguimiento" title="Agendar"> &nbsp;&nbsp; <i class="fas fa-calendar-alt"></i> &nbsp; Agendar </button>
                    <button class="dropdown-item btn btn-outline-secondary btn-sm btn-flat icono-color-principal btnDelSalon" data-toggle="modal" data-target="#ModalEditDatosProspectoSeguimiento" title="Editar"> $nbsp;&nbsp; <i class="fas fa-pencil-alt"></i> &nbsp; Editar </button>
